...
### Starting another paper from this repository

If you want to reuse this customized project page for a new paper, run:

```bash
npm run new-paper -- ../my-new-paper-site --title "Your Paper Title" --repo my-new-paper-site
```

This creates a clean sibling project with the same Astro components, a fresh `src/paper.mdx`, starter `public/image/` and `public/videos/` folders, and the GitHub Pages base path set from `--repo`. See [`NEW_PAPER.md`](./NEW_PAPER.md) for the Chinese quick-start workflow.
